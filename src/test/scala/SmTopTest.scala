...
class SmTopTest extends AnyFlatSpec with ChiselScalatestTester {
  "Sm" should "execute program 1" in {
    test(new SmTop(4, 4, 8, 100, "src/test/scala/kernel1.hex")).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
  ...
  "Sm" should "execute program 2" in {
    test(new SmTop(4, 4, 8, 100, "src/test/scala/kernel2.hex")).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      dut.io.valid.poke(false.B)
      dut.io.data.poke(0.U)
  ...
  "Sm" should "execute program 3" in {
    test(new SmTop(4, 4, 8, 100, "src/test/scala/kernel3.hex")).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      dut.io.valid.poke(false.B)
      dut.io.data.poke(0.U)
